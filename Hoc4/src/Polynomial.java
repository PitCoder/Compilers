...
*/
  ...
  public Polynomial negate(){
    Polynomial npolynomial = new Polynomial();
    for(int i=0; i < polynomial.size(); i++){
      Term term = polynomial.get(i);
      npolynomial.addTerm(new Term(term.getCoefficient()[0] * -1, term.getCoefficient()[1], term.getExponent()));
    }
    return npolynomial;
  }
...
*/
  public void reduce(){
    ...
    for(int i=0;i<polynomial.size();i++){
      ...
      //##Fixed since jdk 6.1 hashmap handle keys with negative numbers
      //if(!exponents.containsKey(polynomial.get(i).getExponent()) && polynomial.get(i).getExponent()>=0){
      if(!exponents.containsKey(polynomial.get(i).getExponent())){
