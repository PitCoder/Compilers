...
public class Machine{
  ...
  public void forCode(){
    boolean decision;
    int savepc = pc;
    execute(((Integer)program.elementAt(savepc)).intValue()); // Init of var
    execute(((Integer)program.elementAt(savepc + 1)).intValue());	/* condition */
    decision = ((Boolean)stack.pop()).booleanValue();

    while(decision) {
      execute(((Integer)program.elementAt(savepc + 3)).intValue()); /* code */
      execute(((Integer)program.elementAt(savepc + 2)).intValue()); /* update stmt */
      execute(((Integer)program.elementAt(savepc + 1)).intValue());	/* testexpr */
      decision = ((Boolean)stack.pop()).booleanValue();
    }
    pc = ((Integer)program.elementAt(savepc + 4)).intValue();
  }
